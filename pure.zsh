...
prompt_pure_preprompt_render() {
	...
	if [[ -n $prompt_pure_vcs_info[branch] ]]; then
		preprompt_parts+=("%K{black} %F{$git_color}"'${prompt_pure_vcs_info[branch]}${prompt_pure_git_dirty}%f %k')
	fi
	# Git pull/push arrows.
	if [[ -n $prompt_pure_git_arrows ]]; then
		preprompt_parts+=('%K{black} %F{cyan}${prompt_pure_git_arrows}%f %k')
	fi
...
prompt_pure_state_setup() {
	...
	if [[ -z $ssh_connection ]] && (( $+commands[who] )); then
		...
		local reIPv6='([a-f0-9]*:){2,}[a-f0-9:]*'  # Simplified, but matches loopback as well (::1).
